...
reforecast_2netcdf_6h.py

This script, for a list of dates for the month in question and
spanning years 2000-2019, will extract the GEFS v12 
6-hourly accumulated precip reforecast data for the list of dates in 
the year and month specified and the lead time and put the 
resulting subsetted forecasts into a netCDF file with
6-h forecast accumulations.  That netCDF file will 
contain all 5 members, considered exchangeable.   This 
script is called by control_reforecast_2netcdf_6h.py
...
def reforecast_2netcdf_6h (date_list, ilead, outfilename):
    ...
    def read_gribdata(idxfilename, gribfilename, validityDate, validityTime):
        ...
        if fexist_grib:
            try:
                #fcstfile = pygrib.open(gribfilename) # if index file nonexistent
    ...
    # ---- define bounding coordinates on Gaussian grid for MDL domain that encompasses
    #      Guam, AK, HI, PR, CONUS
