"""
qmap_reforecast_for_histogram_stats.py cyyyymmddhh clead 
...
from qmapping_spline_fivexfive_flexiknot import \
    qmapping_spline_fivexfive_flexiknot # from f2py    # Lesley, a new-ish f90 routine here to compile
...
def get_quantile_gefsv12(precip_amount, jy, ix, \
    ...
    # Lesley, number_knots_gefsv12 passed in.
    ...
    if precip_amount[jy,ix] == 0.0:
        ...
        if usegamma_gefsv12[jy,ix] == 0:
            ...
            nk = number_knots_gefsv12[jy,ix]  # Lesley, these three lines different
...
def read_precipitation_analyses(master_directory, cyear, \
    cmonth, ctype, iyyyymmddhh_veriftime, missingv): 
    ...
    for imem, cmem in enumerate(cmembers): 
        ...
        for jy in range(ny_gefsv12):
            for ix in range(nx_gefsv12):
                # Lesley, note the number_knots_gefsv12 passed in here.
        ...
        qmapped_precip = \
    	    ...
    	    precip_gefsv12_on_ndfd, spline_info_inv, fraction_zero_ndfd, \
    ...
    if netcdf_thinned == True:
        ...
        if save_chist == True:
            ...
            precip_anal_thinned = read_precipitation_analyses(master_directory_panal, \
                cyyyy, cmonth, ctype, iyyyymmddhh_veriftime, missingv_thinned)
    ...
    if netcdf_upscaled == True:
        ...
        if save_chist == True:
            ...
            precip_anal_upscaled = read_precipitation_analyses(master_directory_panal, \
                cyyyy, cmonth, ctype, iyyyymmddhh_veriftime, missingv_upscaled)
            closest_histogram = np.zeros((25*nmembers, 4), dtype=np.int32)
            nfour = 4
            nmembers_x25 = nmembers*25
            closest_histogram, istat = compute_closest_member_f90( \
