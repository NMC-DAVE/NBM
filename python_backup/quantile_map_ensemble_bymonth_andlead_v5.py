"""
quantile_map_ensemble_bymonth_andlead_v5.py cyyyymmddhh clead 
...
coded by: Tom Hamill, Apr 2021, tom.hamill@noaa.gov 
...
def read_forecast_spline_info(cyyyymmddhh, ccmonth, \
    ...
    return lons_spline_gefsv12_1d, lats_spline_gefsv12_1d, \
        ...
        usegamma_gefsv12, quantile_99, ny_gefsv12, nx_gefsv12, \
...
def get_quantile_gefsv12(precip_amount, jy, ix, \
    spline_info_gefsv12, fraction_zero_gefsv12,\
    usegamma_gefsv12, quantile_99, use99):
    ...
    if precip_amount[jy,ix] == 0.0:
        ...
        if usegamma_gefsv12[jy,ix] == 0:
            ...
            if usegamma_gefsv12[jy,ix] == -1:
                ...
                #print ('usegamma_gefsv12[jy,ix] = ', usegamma_gefsv12[jy,ix])
...
for idate, cyyyymmddhh in enumerate(date_list):
    ...
    if firstdate == True:
        ...
        #print ('max, min quantile_99 = ', \
        #    np.max(quantile_99), np.min(quantile_99))
    ...
    for imem, cmem in enumerate(cmembers): 
        ...
        # print ('shape gefsv12_quantiles_on_ndfd = ',np.shape(gefsv12_quantiles_on_ndfd))
        ...
        #print (qmapping_spline_fivexfive.__doc__)
        qmapped_precip = \
            qmapping_spline_fivexfive(gefsv12_quantiles_on_ndfd, \
    	    precip_gefsv12_on_ndfd, spline_info_inv, \
    	    fraction_zero_ndfd, usegamma_ndfd, use99, offset_on_ndfd, \
            nstride_qmap, ny_ndfd, nx_ndfd)
        ...
        if netcdf_upscaled == True:
            ...
            for istencil in range(25):
                ...
                imu = im.resize(size=(ny_ndfd//10, nx_ndfd//10),resample=Image.BOX)
                precip_ens_qmapped_upscaled[istart+istencil,:,:] = np.transpose(np.asarray(imu))
    ...
    for ithresh, thresh in enumerate(thresholds):
        ...
        if netcdf_upscaled == True:
            ...
            prob_upscaled = generate_probabilities(nmembers*25, precip_ens_qmapped_upscaled, \
                zeros_upscaled, ones_upscaled, thresh, prob_upscaled, \
                ny_upscaled, nx_upscaled)
