""" calculate_brier_reliability_weighted.py cyyyymm clead
"""
...
# ---- read the probability forecasts for this lead time and month
#      saved by the quantile-mapping and weighting routine software.

forecast_directory = '/Volumes/NBM/conus_gefsv12/'+ctype+'/'

if dressit == True:
    infile = forecast_directory + cmonth + cyyyy + \
        '_use99_lead'+clead+'_probabilities_weighted_dressed_'+ctype+'.nc'
else:
    infile = forecast_directory + cmonth + cyyyy + \
        '_use99_lead'+clead+'_probabilities_weighted_'+ctype+'.nc'
print ('reading ', infile)
nc = Dataset(infile, 'r')
probability_qmapped_weighted = nc.variables['probability_qmapped'][:,:,:,:]
print (np.shape(probability_qmapped_weighted))
#sys.exit()
...
contab_forecast_qmapped_weighted = np.zeros((nthresh,32,2), dtype=np.float64)
...
contab_forecast_qmapped_weighted_daily = np.zeros((ndates,nthresh, 32,2), dtype=np.float64)
...
brier_score_qmapped_weighted_overall = np.zeros((nthresh), dtype=np.float64)
...
brier_score_qmapped_weighted_west = np.zeros((nthresh), dtype=np.float64)
...
brier_score_qmapped_weighted_east = np.zeros((nthresh), dtype=np.float64)
...
brier_score_qmapped_weighted_daily = np.zeros((nthresh, ndates), dtype=np.float64)
...
brier_score_qmapped_gridded = \
    np.zeros((nthresh, ny_fcst, nx_fcst), dtype=np.float64)
brier_score_qmapped_weighted_gridded = \
    np.zeros((nthresh, ny_fcst, nx_fcst), dtype=np.float64)
...
for idate, fcst_date in enumerate(yyyymmddhh_fcst_in):
    ...
    for ithresh, thresh in enumerate(thresholds):
        ...
        print ('  max, min probability raw = ', np.min(probability), np.max(probability))
        ...
        print ('  max, min probability qmapped = ', np.min(probability), np.max(probability))
        ...
        # --- quantile mapped and weighted
        
        probability = probability_qmapped_weighted[idate,ithresh,:,:]
        print ('  max, min probability weighted qmapped = ', np.min(probability), np.max(probability))
        #print ('  probability(ny_fcst//2, 0:nx_fcst:10 = ', probability[ny_fcst//2, 0:nx_fcst:10])
        brier_score_qmapped_weighted_overall, brier_score_qmapped_weighted_west, \
            brier_score_qmapped_weighted_east, brier_score_qmapped_weighted_daily, \
            brier_score_qmapped_weighted_gridded, contab_forecast_qmapped_weighted, \
            contab_forecast_qmapped_weighted_daily = \
            compute_brier_score_and_contingency_tables( idate, ithresh, \
            brier_score_qmapped_weighted_overall, brier_score_qmapped_weighted_west, \
            brier_score_qmapped_weighted_east, brier_score_qmapped_weighted_daily, \
            brier_score_qmapped_weighted_gridded, probability, \
            contab_forecast_qmapped_weighted, contab_forecast_qmapped_weighted_daily, \
            precip_anal, thresh, ones, zeros, weight)
...
print ('BSS_qmapped_weighted_overall = ', 1.-brier_score_qmapped_weighted_overall/brier_score_climo_overall)
...
print ('BSS_qmapped_weighted_west = ', 1. - brier_score_qmapped_weighted_west/brier_score_climo_west)
print ('BSS_raw_east = ', 1. - brier_score_raw_east/brier_score_climo_east)
print ('BSS_qmapped_weighted_east = ', 1. - brier_score_qmapped_weighted_east/brier_score_climo_east)


if dressit == True:
    outfile = forecast_directory + cyyyymm + \
        '_lead'+clead+'_'+ctype+'_Brier_contingency_table_weighted_dressed.cPick'
else:
    outfile = forecast_directory + cyyyymm + \
        '_lead'+clead+'_'+ctype+'_Brier_contingency_table_weighted.cPick'
...
print ('brier_score_raw_daily shape = ', \
    np.shape(brier_score_raw_daily))
...
cPickle.dump(brier_score_qmapped_weighted_overall, ouf)
cPickle.dump(brier_score_qmapped_weighted_west, ouf)
cPickle.dump(brier_score_qmapped_weighted_east, ouf)
cPickle.dump(brier_score_qmapped_weighted_daily, ouf)
cPickle.dump(brier_score_qmapped_weighted_gridded, ouf)
cPickle.dump(contab_forecast_qmapped_weighted, ouf)
cPickle.dump(contab_forecast_qmapped_weighted_daily, ouf)
