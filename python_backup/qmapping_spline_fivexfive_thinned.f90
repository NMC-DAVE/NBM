SUBROUTINE qmapping_spline_fivexfive_thinned(gefsv12_quantiles_on_ndfd, &
	precip_gefsv12_on_ndfd, spline_info_inv, fraction_zero_gefsv12_ndfd, &
    fraction_zero_ndfd, usegamma, use99, offset, number_knots, nstride_qmap, &
    nstride,  ny_thinned, nx_thinned, ny_ndfd, nx_ndfd, qmapped_precip_thinned)

! compile with the following python command
    
! f2py --opt='-O4' --opt='-Wno-tabs' -c -m qmapping_spline_fivexfive_thinned qmapping_spline_fivexfive_thinned.f90 cumgam.f90 gamma_inc.f90 error_f.f90 error_fc.f90 exparg.f90 gam1.f90 ipmpar.f90 pgamma.f90 dgamma.f90 qgamma.f90 rlog.f90 rexp.f90 dnorm.f90 pnorm.f90 qnorm.f90 gamma.f90 splev.f fpbspl.f
	
! Quantile mapping on a thinned grid using splines that predict the analyzed precipitation 
...
INTEGER, INTENT(IN) :: ny_ndfd, nx_ndfd, ny_thinned, nx_thinned, nstride_qmap, nstride
...
REAL*8, INTENT(OUT), DIMENSION(25, ny_thinned, nx_thinned) ::	qmapped_precip_thinned

! f2py intent(in) nstride_qmap, nstride,  ny_thinned, nx_thinned, ny_ndfd, nx_ndfd 
...
! f2py intent(out) qmapped_precip_thinned
...
! f2py depend(25,ny_thinned, nx_thinned) qmapped_precip_thinned
...
DO jy = 1, ny_ndfd-1, nstride
	jyout = 1 + jy/nstride

	! ---- set the min and max j location for stencil loop.
	
	jymin = jy - 2*nstride_qmap
	jymax = jy + 2*nstride_qmap
	
	DO ix = 1, nx_ndfd-1, nstride
		ixout = 1 + ix/nstride
			
		! ---- set the min and max i location for stencil loop.
		ixmin = ix - 2*nstride_qmap
		ixmax = ix + 2*nstride_qmap
		istencil = 1

		!print *,'jy, ix, jyout, ixout, ny_ndfd, nx_ndfd, ny_thinned, nx_thinned = ', &
		!	jy, ix, jyout, ixout, ny_ndfd, nx_ndfd, ny_thinned, nx_thinned
		...
		DO jyf = jymin, jymax, nstride_qmap
			jyuse = jyf
			IF (jyf .lt. 1) jyuse = 1
			IF (jyf .gt. ny_ndfd) jyuse = ny_ndfd
			DO ixf = ixmin, ixmax, nstride_qmap
				ixuse = ixf
				IF (ixf .lt. 1) ixuse = 1
				IF (ixf .gt. nx_ndfd) ixuse = nx_ndfd
				!print *,'  jyuse, ixuse, jy, ix, ny_ndfd, nx_ndfd, ny_thinned, nx_thinned  = ', &
				!	jyuse,ixuse, jy,ix, ny_ndfd, nx_ndfd, ny_thinned, nx_thinned
				...
				qmapped_precip_thinned(istencil,jyout,ixout) = qmp
				istencil = istencil+1
				
			END DO ! ixf stencil point
		END DO ! jyf stencil point
				
	END DO ! ix input grid point
END DO ! jy input grid point


RETURN
END SUBROUTINE qmapping_spline_fivexfive_thinned
