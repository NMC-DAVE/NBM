"""
plot_bss_reliability_precip_1month.py cyyyymm cmonth cndays 
    where cseason == 'cool' or 'warm'

cyyyymm = sys.argv[1] # warm, cool
cmonth = sys.argv[2] # 'Jan' etc.
cndays = sys.argv[3]
"""
...
def calc_BSS_relia(brier_score_overall, brier_score_climo_overall, \
    ...
    #print ('brier_score_daily shape = ', np.shape(brier_score_daily))
...
def bootstrap_reliability(contab_forecast_daily, ndays_total, \
    ...
    for iresa in range(nresa):
        ...
        ix = np.random.uniform(low=0.0, high=float(ncats-1), \
...
cmonth = sys.argv[2] # 'Jan' etc.
cndays = sys.argv[3]
ndays_total = int(cndays)
#cleads = ['012','024','036','048','060','072',\
#    '084','096','108','120','132','144','156',\
