SUBROUTINE qmapping_spline_fivexfive_probs(gefsv12_quantiles_on_ndfd, &
	precip_gefsv12_on_ndfd, spline_info_inv, &
	fraction_zero_ndfd, usegamma, use99, offset, &
	ny_ndfd, nx_ndfd, thresholds, nthresholds, nstride, &
	qmapped_precip, probability_forecast)

! sudo f2py --opt='-O4' --opt='-Wno-tabs' -c -m qmapping_spline_fivexfive_probs qmapping_spline_fivexfive_probs.f90 cumgam.f90 gamma_inc.f90 error_f.f90 error_fc.f90 exparg.f90 gam1.f90 ipmpar.f90 pgamma.f90 dgamma.f90 qgamma.f90 rlog.f90 rexp.f90 dnorm.f90 pnorm.f90 qnorm.f90 gamma.f90 splev.f fpbspl.f
	
! this version does a few things different from previous version.
! It is, per older articles, going to use a stencil of 
! the surrounding grid points and their quantiles as 
! expanded input into the quantile mapping.   It will
! also generate probabilities for this member here.  It still returns
! the quantile-mapped member at the center point of the stencil 
! only.
...
REAL*8, INTENT(IN), DIMENSION(nthresholds) :: thresholds
...
! f2py depend(nthresholds, ny_ndfd, nx_ndfd) probability_forecast
...
RETURN
END SUBROUTINE qmapping_spline_fivexfive_probs
