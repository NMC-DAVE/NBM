"""
save_sorted_forecasts_analyses.py
...
for imonth, cmonth in enumerate(cmonths):
    ...
    forecast_validdates_sorted = np.zeros((ndates_valid, nlats, nlons), dtype=np.float64)
    analysis_validdates_sorted = np.zeros((ndates_valid, nlats, nlons), dtype=np.float64)
    ...
    # ---- for each grid point, sort the forecasts lowest to highest, and
    #      order the observations in a similar manner.   Save these
    #      sorted arrays to files for later use in an analog approach.
    
    print ('   sorting')
    for jy in range(nlats):
        #print ('jy = ', jy)
        for ix in range(nlons):
            f = forecast_validdates[:,jy,ix]+np.random.randn(ktr)*0.0001
            a = analysis_validdates[:,jy,ix]
            idxs = np.argsort(f)
            forecast_validdates_sorted[:,jy,ix] = f[idxs]
            analysis_validdates_sorted[:,jy,ix] = a[idxs]
    
    outfile = cpath_gefsv12+'forecast_analyzed_sorted_'+cmonth+\
        '_lead='+clead+'.cPick'
    print ('   writing to ', outfile)
    ouf = open(outfile,'wb')
    cPickle.dump(forecast_validdates_sorted, ouf)
    cPickle.dump(analysis_validdates_sorted, ouf)
    ouf.close()
    print ('   done writing')
