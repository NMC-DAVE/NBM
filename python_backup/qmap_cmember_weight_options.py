"""
qmap_cmember_weight.py cyyyymmddhh clead 
...
from qmapping_spline_fivexfive_flexiknot_test import \
    qmapping_spline_fivexfive_flexiknot_test # from f2py  
...
master_directory_histogram_upscaled = '/Volumes/NBM/conus_gefsv12/upscaled/chist/'
...
if netcdf_upscaled == True:
    # Lesley, 10/28/21 change to this routine
...
# ---- read the closest-member histograms for upscaled

if netcdf_upscaled == True:
    # Lesley, 10/28/21 change to this routine
    histogram_thresholds, closest_member_histogram_upscaled = \
        read_closest_member_histogram(master_directory_histogram_upscaled, \
        'upscaled', cmonth, clead)
...
for idate, cyyyymmddhh in enumerate(date_list):
    ...
    if firstdate == True:
            ...
            if use99 == False:
                if dressit == True:
                    outfile = master_directory_fullfield_output + ccmonth + cyyyy + \
                        '_lead'+clead+'_probabilities_weighted_dressed_fullfield.nc'
                else:
                    outfile = master_directory_fullfield_output + ccmonth + cyyyy + \
                        '_lead'+clead+'_probabilities_weighted_fullfield.nc'
            else:
                if dressit == True:
                    outfile = master_directory_fullfield_output + ccmonth + cyyyy + \
                        '_use99_lead'+clead+'_probabilities_weighted_dressed_fullfield.nc'
                else:
                    outfile = master_directory_fullfield_output + ccmonth + cyyyy + \
                        '_use99_lead'+clead+'_probabilities_weighted_fullfield.nc'
            ncname = 'nc_fullfield'
        ...
        prob_qmapped_weighted_out = np.zeros((nthresholds, ny_ndfd, nx_ndfd), \
            dtype=np.float64)
        prob_qmapped_weighted_work = np.zeros((nthresholds, ny_ndfd, nx_ndfd), \
            dtype=np.float64)
        ...
        if netcdf_upscaled == True:
            ...
            precip_ens_qmapped_upscaled = np.zeros((25*nmembers, ny_upscaled, nx_upscaled), \
                dtype=np.float32)
            prob_qmapped_weighted_upscaled = np.zeros((nthresholds, ny_upscaled, nx_upscaled), \
                dtype=np.float32)
            prob_upscaled = np.zeros((nthresholds, ny_upscaled, nx_upscaled), dtype=np.float64)
            ...
            if use99 == False:
                if dressit == True:
                    outfile = master_directory_upscaled_output + ccmonth + cyyyy + \
                        '_lead'+clead+'_probabilities_weighted_dressed_upscaled.nc'
                else:
                    outfile = master_directory_upscaled_output + ccmonth + cyyyy + \
                        '_lead'+clead+'_probabilities_weighted_upscaled.nc'
            else:
                if dressit == True:
                    outfile = master_directory_upscaled_output + ccmonth + cyyyy + \
                        '_use99_lead'+clead+'_probabilities_weighted_dressed_upscaled.nc'
                else:
                    outfile = master_directory_upscaled_output + ccmonth + cyyyy + \
                        '_use99_lead'+clead+'_probabilities_weighted_upscaled.nc'
            ncname = 'nc_upscaled'
            istat = initialize_netCDF_probfiles(outfile, ncname, \
    ...
    if netcdf_fullfield == True:
        ...
        prob_qmapped_weighted_out = \
            ...
            closest_member_histogram_thinned, dressit)
    
        print ('   min, max, mean prob fullfield = ',\
    ...
    if netcdf_upscaled == True:
        ...
        prob_qmapped_weighted_upscaled = \
            generate_probabilities(nmembers*25, thresholds, \
            ensmean_qmapped_upscaled, precip_ens_qmapped_upscaled, \
            prob_upscaled, ny_upscaled, nx_upscaled, histogram_thresholds, \
            closest_member_histogram_upscaled, dressit)
        print ('   min, max, mean prob upscaled = ',\
            np.min(prob_qmapped_weighted_upscaled), \
            np.max(prob_qmapped_weighted_upscaled), \
            np.mean(prob_qmapped_weighted_upscaled))
    ...
    if netcdf_fullfield == True:
        ...
        istat = write_probabilities_to_netcdf(outfile, ncname, idate, \
            cyyyymmddhh, cyyyymmddhh_fcst, prob_qmapped_weighted_out, \
            ensmean_qmapped_ndfd)
    ...
    if netcdf_upscaled == True:
        ...
        istat = write_probabilities_to_netcdf(outfile, ncname, idate, \
            cyyyymmddhh, cyyyymmddhh_fcst, prob_qmapped_weighted_upscaled, \
