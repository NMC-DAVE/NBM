"""
qmap_gefsv12_ccpa_spline_v2.py cyyyymmddhh clead
...
def get_domain_subset_of_gefs(cyyyymmddhh, cmem, clead, jmin, \
    ...
    # ---- further subset the data to the 
...
mswep_directory = '/Volumes/Backup Plus/ccpa/'
...
print ('interpolating forecast of interest to NDFD grid')
...
riclose = np.zeros((ny_mswep, nx_mswep), dtype=np.float64)
rjclose = np.zeros((ny_mswep, nx_mswep), dtype=np.float64)
if readclosest == False:
    ...
    for jy in range(ny_mswep):
        ...
        for ix in range(nx_mswep):
            ...
            ridxx = nx_gefsv12*frac
            if idxx < 0 or idxx > nx_gefsv12-1:
                if idxx < 0: 
                    idxx = 0
                    ridxx = 0.0
                if idxx > nx_gefsv12-1: 
                    idxx = nx_gefsv12-1
                    ridxx = nx_gefsv12-1.0
            iclose[jy,ix] = idxx
            riclose[jy,ix] = ridxx
            ...
            ridxy = ny_gefsv12*frac

            if idxy < 0 or idxy > ny_gefsv12-1:
                if idxy < 0: 
                    idxy = 0
                    ridxy = 0.0
                if idxy > ny_gefsv12-1: 
                    idxy = ny_gefsv12-1
                    ridxy = ny_gefsv12-1.0
            jclose[jy,ix] = idxy
            rjclose[jy,ix] = ridxy
    ...
    cPickle.dump(riclose, ouf)
    cPickle.dump(rjclose, ouf)    
    ouf.close()
else:
    ...
    riclose = cPickle.load(inf)
    rjclose = cPickle.load(inf)
    inf.close()
    
#print ('sample of iclose and jclose')
#print ('iclose[7*ny_mswep//8,0:nx_mswep//10] = ',iclose[7*ny_mswep//8,0:nx_mswep//10] )
#print ('jclose[7*ny_mswep//8,0:nx_mswep//10] = ',jclose[7*ny_mswep//8,0:nx_mswep//10] )
...
#for jy in range(291,292):
    ...
    for ix in range(nx_mswep):
        ...
        if interpolate == False:
            ...
            iminus = iclose[jy,ix]
            jminus = jclose[jy,ix]
            iplus = min([iminus+1,nx_gefsv12-1])
            jplus = min([jminus+1,ny_gefsv12-1])
            xfraction = riclose[jy,ix] - iclose[jy,ix]
            yfraction = rjclose[jy,ix] - jclose[jy,ix]
            
            pgefs_jminus_iminus = precip_realtime[jminus,iminus]
            pgefs_jplus_iminus = precip_realtime[jplus,iminus]
            pgefs_jminus_iplus = precip_realtime[jminus,iplus]
            pgefs_jplus_iplus = precip_realtime[jplus,iplus]
        
            gefsv12_quantile_jminus_iminus = get_quantile_gefsv12(pgefs_jminus_iminus, jminus, \
                iminus, spline_info_gefsv12, fraction_zero_gefsv12)
            gefsv12_quantile_jplus_iminus = get_quantile_gefsv12(pgefs_jplus_iminus, jplus, \
                iminus, spline_info_gefsv12, fraction_zero_gefsv12)
            gefsv12_quantile_jminus_iplus = get_quantile_gefsv12(pgefs_jminus_iplus, jminus, \
                iplus, spline_info_gefsv12, fraction_zero_gefsv12)
            gefsv12_quantile_jplus_iplus = get_quantile_gefsv12(pgefs_jplus_iplus, jplus, \
                iplus, spline_info_gefsv12, fraction_zero_gefsv12)
                
            qm_jminus_iminus = get_qmapped_precip(gefsv12_quantile_jminus_iminus, \
                pgefs_jminus_iminus, jy, ix, spline_info_inv, indices_to_query_mswep, \
                fraction_zero_mswep)
            qm_jplus_iminus = get_qmapped_precip(gefsv12_quantile_jplus_iminus, \
                pgefs_jplus_iminus, jy, ix, spline_info_inv, indices_to_query_mswep, \
                fraction_zero_mswep)
            qm_jminus_iplus = get_qmapped_precip(gefsv12_quantile_jminus_iplus, \
                pgefs_jminus_iplus, jy, ix, spline_info_inv, indices_to_query_mswep, \
                fraction_zero_mswep)
            qm_jplus_iplus = get_qmapped_precip(gefsv12_quantile_jplus_iplus, \
                pgefs_jplus_iplus, jy, ix, spline_info_inv, indices_to_query_mswep, \
                fraction_zero_mswep)                
                
            qmapped_precip[jy,ix] = \
                (1.0 -xfraction)*(1.0-yfraction)*qm_jminus_iminus + \
                xfraction*(1.0-yfraction)*qm_jplus_iminus + \
                (1.0 -xfraction)*yfraction*qm_jminus_iplus + \
                xfraction*yfraction*qm_jplus_iplus 
