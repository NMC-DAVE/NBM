SUBROUTINE qmapping_spline_flexiknot(gefsv12_quantiles_on_ndfd, &
...
! f2py --opt='-O4' --opt='-Wno-tabs' -c -m qmapping_spline_flexiknot qmapping_spline_flexiknot.f90 cumgam.f90 gamma_inc.f90 error_f.f90 error_fc.f90 exparg.f90 gam1.f90 ipmpar.f90 pgamma.f90 dgamma.f90 qgamma.f90 rlog.f90 rexp.f90 dnorm.f90 pnorm.f90 qnorm.f90 gamma.f90 splev.f fpbspl.f
...
DO jy = 1, ny_ndfd
	DO ix = 1, nx_ndfd
		...
		!DO jy = 1021, 1021		
		!	DO ix = 1284-50, 1284+50, 5
		!print *,'========= jy, ix = ',jy,ix
		!print *,'   precip_gefsv12_on_ndfd(jy,ix) = ', precip_gefsv12_on_ndfd(jy,ix)
		!print *,'   fraction_zero_gefsv12_ndfd(jy,ix) = ',fraction_zero_gefsv12_ndfd(jy,ix) 
		!print *,'   gefsv12_quantiles_on_ndfd(jy,ix) = ', gefsv12_quantiles_on_ndfd(jy,ix)
		!print *,'   offset(jy,ix) = ', offset(jy,ix)
		...
		ELSE
	    	...
	    	IF (usegamma(jy,ix) .eq. 1) THEN 
	        	...
	        	scale = 1.0 / beta
				cum = qpositive 
				!PRINT *, '   alpha, beta, scale, cum = ', alpha, beta, scale, cum 
			...
			ENDIF
            !PRINT *,'   qmp before = ', qmp
			IF (use99 .eqv. .TRUE. .and. offset(jy,ix) .gt. 0.0) THEN
				qmp = qmp + offset(jy,ix)
				!PRINT *,'   qmp after, offset = ', qmp, offset(jy,ix)
			ENDIF
		ENDIF
			
		if (isnan(qmp))	qmp = precip_gefsv12_on_ndfd(jy,ix)
		qmp=MAX(0.0, qmp)
		!print *,'   final quantile-mapped value = ',qmp
...
RETURN
END SUBROUTINE qmapping_spline_flexiknot
