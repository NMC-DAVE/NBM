"""
plot_bss_reliability_weighted_precip.py ctype cseason
    where ctype == 'thinned' or 'upscaled'
        cseason == 'cool' or 'warm'
...
def calc_BSS_relia(brier_score_raw_overall, brier_score_qmapped_overall, \
    ...
    brier_score_qmapped_west, brier_score_qmapped_east, \
    brier_score_qmapped_weighted_west, brier_score_qmapped_weighted_east, \
    brier_score_climo_west, brier_score_climo_east, \
    brier_score_raw_daily, brier_score_qmapped_daily, \
    brier_score_qmapped_weighted_daily,\
    brier_score_climo_daily, brier_score_raw_gridded, \
    brier_score_qmapped_gridded, brier_score_qmapped_weighted_gridded, \
    brier_score_climo_gridded, \
    contab_forecast_raw, contab_forecast_qmapped, contab_forecast_qmapped_weighted,\
    ...
    for ithresh in range(nthresh): 
        ...
        BSS_qmapped_weighted[ilead,ithresh] = 1. - \
            brier_score_qmapped_weighted_overall[ithresh] / \
            brier_score_climo_overall[ithresh]
        BSS_qmapped_weighted_west[ilead,ithresh] = 1. - \
            brier_score_qmapped_weighted_west[ithresh] / \
            brier_score_climo_west[ithresh]
        BSS_qmapped_weighted_east[ilead,ithresh] = 1. - \
            brier_score_qmapped_weighted_east[ithresh] / \
            brier_score_climo_east[ithresh]
            ...
            for ix in range(nx):
                if brier_score_climo_gridded[ithresh,jy,ix] > 0.0:
                    ...
                    BSS_qmapped_gridded[ilead,ithresh,jy,ix] = \
                        ...
                        brier_score_climo_gridded[ithresh,jy,ix]
                    BSS_qmapped_weighted_gridded[ilead,ithresh,jy,ix] = \
                        1.0 - brier_score_qmapped_weighted_gridded[ithresh,jy,ix] / \
                        brier_score_climo_gridded[ithresh,jy,ix]
                else:
                    BSS_raw_gridded[ilead,ithresh,jy,ix] = -99.99
                    BSS_qmapped_gridded[ilead,ithresh,jy,ix] = -99.99
                    BSS_qmapped_weighted_gridded[ilead,ithresh,jy,ix] = -99.99
        ...
        nsamps_total_qmapped_weighted = \
            np.sum(contab_forecast_qmapped_weighted[ithresh,:,:])
        for icat in range(ncats):
            ...
            frequse_qmapped[ilead,ithresh,icat] = \
                ...
                float(nsamps_total_qmapped)
            if np.sum(contab_forecast_qmapped[ithresh,icat,:]) > 50:
                relia_qmapped[ilead,ithresh,icat] = \
                    contab_forecast_qmapped[ithresh,icat,1] / \
                    np.sum(contab_forecast_qmapped[ithresh,icat,:])
            else:
                relia_qmapped[ilead,ithresh,icat] = -99.99
                
            frequse_qmapped_weighted[ilead,ithresh,icat] = \
                np.sum(contab_forecast_qmapped_weighted[ithresh,icat,:]) / \
                float(nsamps_total_qmapped_weighted)
            if np.sum(contab_forecast_qmapped_weighted[ithresh,icat,:]) > 50:
                relia_qmapped_weighted[ilead,ithresh,icat] = \
                    contab_forecast_qmapped_weighted[ithresh,icat,1] / \
                    np.sum(contab_forecast_qmapped_weighted[ithresh,icat,:])
            else:
                relia_qmapped_weighted[ilead,ithresh,icat] = -99.99
        ...
        relia_qmapped_weighted_05[ilead,ithresh,:], \
            relia_qmapped_weighted_95[ilead,ithresh,:] = \
            bootstrap_reliability(contab_forecast_qmapped_weighted_daily, \
            ndays_total, ithresh, ncats)
 
    BSSes_raw = np.zeros((nthresh), dtype=np.float32)
    BSSes_qmapped = np.zeros((nthresh), dtype=np.float32)
    BSSes_qmapped_weighted = np.zeros((nthresh), dtype=np.float32)
    print ('brier_score_raw_daily shape = ', np.shape(brier_score_raw_daily))
    print ('> 10 mm forecast skill by day  raw, qmapped, qmapped&weighted')
    print ('ndays_total = ', ndays_total)
    for idate in range(ndays_total):
        BSSes_raw[:] = 1. -  brier_score_raw_daily[idate,:] / \
            brier_score_climo_daily[idate,:]
        BSSes_qmapped[:] = 1. -  brier_score_qmapped_daily[idate,:] / \
            brier_score_climo_daily[idate,:]
        BSSes_qmapped_weighted[:] = 1. -  \
            brier_score_qmapped_weighted_daily[idate,:] / \
            brier_score_climo_daily[idate,:]
        #print (idate, yyyymmddhh_init[idate], BSSes_raw[3],BSSes_qmapped[3],\
        #    BSSes_qmapped_weighted[3], fmt=["%d","%d","%6.2f","%6.2f", "%6.2f"])
            
        print ("{0:4},{1:10},{2:8.2},{3:8.2},{4:8.2}".format(idate, \
            yyyymmddhh_init_daily[idate], BSSes_raw[3], BSSes_qmapped[3], \
            BSSes_qmapped_weighted[3]))

    
    return BSS_raw, BSS_raw_05, BSS_raw_95, \
        BSS_raw_west, BSS_raw_east, BSS_raw_gridded, \
        relia_raw, relia_raw_05, relia_raw_95, frequse_raw, \
        BSS_qmapped, BSS_qmapped_05, BSS_qmapped_95, BSS_qmapped_west, \
        BSS_qmapped_east, BSS_qmapped_gridded, \
        BSS_qmapped_weighted, BSS_qmapped_weighted_05, \
        BSS_qmapped_weighted_95, BSS_qmapped_weighted_west, \
        BSS_qmapped_weighted_east, BSS_qmapped_weighted_gridded,\
        relia_qmapped, relia_qmapped_05, \
...
cseason_in = sys.argv[2] # warm, cool
dressit = False
...
#forecast_directory = '/Volumes/NBM/gefsv12/'+ctype+'/'
...
for ilead, clead in enumerate(cleads):
    ...
    for item, cyyyymm in enumerate(cyyyymm_list):
        ...
        if dressit == False:
            infile = forecast_directory + cyyyymm + \
                '_lead'+clead+'_'+ctype+ \
                '_Brier_contingency_table_weighted.cPick'
        else:
            infile = forecast_directory + cyyyymm + \
                '_lead'+clead+'_'+ctype+\
                '_Brier_contingency_table_weighted_dressed.cPick'
        ...
        print ('np.shape(brier_score_raw_daily_in) = ', \
            np.shape(brier_score_raw_daily_in))
    ...
    BSS_raw, BSS_raw_05, BSS_raw_95, \
        ...
        relia_qmapped_weighted, relia_qmapped_weighted_05, \
        relia_qmapped_weighted_95, frequse_qmapped_weighted = \
        calc_BSS_relia(brier_score_raw_overall, \
        brier_score_qmapped_overall, brier_score_qmapped_weighted_overall,\
        brier_score_climo_overall, brier_score_raw_west, brier_score_raw_east, \
        brier_score_qmapped_west, brier_score_qmapped_east, \
        brier_score_qmapped_weighted_west, brier_score_qmapped_weighted_east, \
        brier_score_climo_west, brier_score_climo_east, brier_score_raw_daily, \
        brier_score_qmapped_daily, brier_score_qmapped_weighted_daily,\
        brier_score_climo_daily, brier_score_raw_gridded, \
        brier_score_qmapped_gridded, brier_score_qmapped_weighted_gridded, \
        brier_score_climo_gridded, contab_forecast_raw, \
        contab_forecast_qmapped, contab_forecast_qmapped_weighted,\
        contab_forecast_raw_daily, contab_forecast_qmapped_daily, \
        contab_forecast_qmapped_weighted_daily,\
        ndays_total, nthresh, ncats, ny, nx, ilead, BSS_raw, BSS_raw_05, \
        BSS_raw_95, BSS_raw_west, BSS_raw_east, BSS_raw_gridded,\
        ...
        relia_qmapped, relia_qmapped_05, relia_qmapped_95, frequse_qmapped, \
        relia_qmapped_weighted, relia_qmapped_weighted_05, \
        relia_qmapped_weighted_95, frequse_qmapped_weighted, yyyymmddhh_init_daily)
    ...
    for ithresh in range(nthresh):
        ...
        relia_m_qmapped_error = \
            ma.array((relia_qmapped_95[ilead,ithresh,:]-relia_qmapped_05[ilead,ithresh,:])/2.)
        relia_m_qmapped_error = \
        ...
        relia_m_qmapped_weighted = \
            ma.masked_where(relia_m_qmapped_weighted < 0.0, relia_m_qmapped_weighted)
        ...
        if dressit == False:
            ...
            a1.plot(probs,100.*relia_m_qmapped_weighted,\
                'o-',color='LimeGreen',linewidth=1.5,\
        ...
        a2.bar(probs-0.9,frequse_raw[ilead,ithresh,:],width=0.7,bottom=0.001,\
            log=True,color='red',edgecolor='None',align='center')
        a2.bar(probs,frequse_qmapped[ilead,ithresh,:],width=0.7,bottom=0.001,\
            log=True,color='RoyalBlue',edgecolor='None',align='center')
        a2.bar(probs+0.9,frequse_qmapped_weighted[ilead,ithresh,:],width=0.7,bottom=0.001,\
        ...
        if dressit == False:
            plot_title = 'relia_weighted_'+ccseason+'_'+ccthresholds[ithresh]+'_'+\
                cleads[ilead]+'h_'+ctype+'.png'
        else:
            plot_title = 'relia_weighted_dressed'+ccseason+'_'+\
                ccthresholds[ithresh]+'_'+cleads[ilead]+'h_'+ctype+'.png'
