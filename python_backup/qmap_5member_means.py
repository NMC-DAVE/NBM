"""
qmap_5member_means.py cyyyymmddhh clead 
...
quantile mapping to 5-member means.  It writes these
...
def read_forecast_spline_info(cyyyymmddhh, ccmonth, \
    ...
    infile = master_directory_forecast_spline + \
        ccmonth + '_conus_GEFSv12_mean_spline_info_h' + clead_use + '.nc' 
...
def initialize_netCDF_meanfile(outfile, ncname, ny, nx, \
    lats_in, lons_in):
    ...
    ensmean_raw = ncout.createVariable('ensmean_raw',\
        'f4',('sample','yf','xf',),
        zlib=True,least_significant_digit=3)
    ensmean_raw.units = "mm"
    ensmean_raw.long_name = 'Raw ensemble-mean '+\
    ...
    ncout.title = 'NDFD CONUS gridded ensemble-mean precipitation, raw and quantile mapped'
...
def fraczero_possamps(nsamps, precip):
    """
    from the vector input sample precip, define the fraction of
...
def get_quantile_gefsv12(precip_amount, precip_gefsv12_on_ndfd, \
    ...
    #for ix in range(0,nx_gefsv12):
    #    print (ix, precip_amount[ny_gefsv12//6,ix], \
    #        gefsv12_quantiles[nx_gefsv12//6,ix], usegamma_gefsv12[nx_gefsv12//6,ix])
    ...
    #for ix in range(0,nx_ndfd,10):
    #    print (ix, precip_gefsv12_on_ndfd[ny_ndfd//6,ix], \
    #        gefsv12_quantiles_on_ndfd[ny_ndfd//6,ix])
...
def write_ensmeans_to_netcdf (outfile, ncname, idate, cyyyymmddhh, \
    cyyymmddhh_fcst,  ensmean_raw_out,  ensmean_qmapped_out):    
    
    """ write the raw and quantile-mapped ensemble means to 
    a netCDF file """
...
nstride_qmap = 12 + int(clead)//18 # 15 # for using grid point stencil. 2.5 km pixels, 15 goes out to +/- 75 km
dressit = False
...
for idate, cyyyymmddhh in enumerate(date_list):
    ...
    if firstdate == True:
        ...
        outfile = master_directory_fullfield_output + cyyyymmddhh + \
            '_lead'+clead+'_ensmeans_fullfield.nc'
        ncname = 'nc_fullfield'
        istat = initialize_netCDF_meanfile(outfile, ncname, \
            ny_ndfd, nx_ndfd, lats_ndfd, lons_ndfd)
            
        # ---- set up full output grids and working arrays

        precip_mean_raw_ndfd = np.zeros((ny_ndfd, nx_ndfd), \
            dtype=np.float32)
        precip_mean_qmapped_ndfd = np.zeros((ny_ndfd, nx_ndfd), \
            dtype=np.float32)
    ...
    # ========================================================================
    # ---- read in GEFS members, make means of 5, interpolate to NDFD grid and quantile map
    # ========================================================================
    ...
    for imem, cmem in enumerate(cmembers): 
        ...
        if imem == 0:
            precip_ens_realtime = np.zeros((nmembers, ny_gefsv12, \
                nx_gefsv12), dtype=np.float32)
        precip_ens_realtime[imem,:,:] = precip_realtime[:,:]
                
    for igroup in range(6): # members 0-29 in groups of 5
        istart = igroup*5
        print ('----- processing igroup = ', igroup, ' istart = ', istart)
        precip_mean = np.mean(precip_ens_realtime[istart:istart+5,:,:], axis=0)
        ...
        precip_realtime_flipud = np.flipud(precip_mean) 
        ...
        gefsv12_quantiles_on_ndfd, offset_on_ndfd = get_quantile_gefsv12(\
            precip_mean, precip_gefsv12_on_ndfd, spline_info_gefsv12, \
        ...
        # ---- add this raw and quantile-mapped value to a running tally for later 
        #      calculation of a mean
    
        print ('   add to tally of the mean precip')
        precip_mean_raw_ndfd = precip_mean_raw_ndfd + precip_gefsv12_on_ndfd
        precip_mean_qmapped_ndfd = precip_mean_qmapped_ndfd + qmapped_precip
            
    # ---- now get mean over all groups
    
    print ('getting means over all groups')
    precip_mean_raw_ndfd = precip_mean_raw_ndfd / 6.
    precip_mean_qmapped_ndfd = precip_mean_qmapped_ndfd / 6.
    
    # ========================================================================
    # ---- write the means to netCDF file.
    ...
    if netcdf_fullfield == True:
        ...
        istat = write_ensmeans_to_netcdf (outfile, ncname, idate, cyyyymmddhh, \
            cyyyymmddhh_fcst,  precip_mean_raw_ndfd, precip_mean_qmapped_ndfd)
