"""
qmap_gefsv12_ensemble_ccpa_spline_nc_v2.py cyyyymmddhh clead 
...
def get_quantile_gefsv12(precip_amount, jy, ix, \
    ...
    if precip_amount[jy,ix] == 0.0:
        ...
        if usegamma_gefsv12[jy,ix] == 0:
            ...
            if quantile > 0.9999: quantile = 0.9999    
...
prob_raw_out = np.zeros((nthresholds, ny_ndfd, nx_ndfd), dtype=np.float32)
prob_qmapped_out = np.zeros((nthresholds, ny_ndfd, nx_ndfd), dtype=np.float32)
...
for imem, cmem in enumerate(cmembers): 
    ...
    #      98th percentile of forecasts, and latitude array to 
    #      NDFD grid. Set up output grids.
    ...
    if cmem == 'c00':
        ...
        quantile_98_flipped = np.flipud(quantile_98)
        quantile_98_on_ndfd = interp(quantile_98_flipped, \
            lons_1d_realtime, lats_1d_realtime_flipud, \
            lons_ndfd, lats_ndfd, checkbounds=False, \
            masked=False, order=1)
    ...
    for jy in range(ny_gefsv12):
        for ix in range(nx_gefsv12):
            gefsv12_quantiles[jy,ix], offset[jy,ix] = \
                get_quantile_gefsv12(precip_realtime, jy, \
...
# ---- generate probabilities, raw and quantile mapped
