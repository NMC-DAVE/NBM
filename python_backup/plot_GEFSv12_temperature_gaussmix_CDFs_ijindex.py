"""
plot_GEFSv12_temperature_gaussmix_CDFs_ijindex.py
...
for idate, date in enumerate(date_list_anal):
    ...
    if fexist1 == True:
        ...
        if idate == 0: 
            ...
            print (jmin, imin, np.shape(lons), np.shape(lats))
            rlon = lons[0,imin]
            rlat = lats[jmin,0]
            clon = str(rlon)
            clat = str(rlat)
...
for idate, date in enumerate(date_list_anal):
    if datevalid_indices[idate] == 1:
        ...
        datevalid.append(date)
        print (idate,date, forecast_3d[idate,jmin,imin] - analysis_3d[idate,jmin,imin],\
             forecast_3d[idate,jmin,imin], analysis_3d[idate,jmin,imin])
        ktr = ktr+1
...
ax.plot(forecast_sample_sorted,fweights[0]*CDF_forecast1,color='Blue',lw=0.5)
ax.plot(forecast_sample_sorted,fweights[1]*CDF_forecast2,color='Blue',lw=0.5)
ax.plot(forecast_sample_sorted,fweights[2]*CDF_forecast3,color='Blue',lw=0.5)
...
ax.plot(analysis_sample_sorted,aweights[0]*CDF_analysis1,color='Blue',lw=0.5)
ax.plot(analysis_sample_sorted,aweights[1]*CDF_analysis2,color='Blue',lw=0.5)
ax.plot(analysis_sample_sorted,aweights[2]*CDF_analysis3,color='Blue',lw=0.5)
...
figname = 'QQplot_'+cmonth+'_lon='+clon+'_lat='+clat+'_lead='+clead+'.pdf'
plt.savefig(figname,dpi=400)
print ('Plot done', figname)
plt.close()
    
    
# ---- scatterplot of F vs. O

print (ktr,len(datevalid))
f = plt.figure(figsize=(6.5,6.))

ax = f.add_axes([.15,.15,.8,.74])
ax.set_title('Scatterplot for '+cmonth+',\n lon = '+clon+' lat = '+clat+' lead = '+clead+'h',fontsize=15)
ax.plot([15.,35.],[15.,35.],'k-',lw=1)
for i in range(ktr):
    yyyymmddhh = str(datevalid[i])
    iyear = int(yyyymmddhh[0:4])
    if iyear < 2005:
        ax.scatter(forecast_validdates[i,jmin,imin],analysis_validdates[i,jmin,imin],s=0.4,color='Red')
    elif iyear >= 2005 and iyear < 2010:
        ax.scatter(forecast_validdates[i,jmin,imin],analysis_validdates[i,jmin,imin],s=0.4,color='Black')
    elif iyear >= 2010 and iyear < 2015:
        ax.scatter(forecast_validdates[i,jmin,imin],analysis_validdates[i,jmin,imin],s=0.4,color='RoyalBlue')
    else:
        ax.scatter(forecast_validdates[i,jmin,imin],analysis_validdates[i,jmin,imin],s=0.4,color='LimeGreen')
        
ax.grid(True,lw=0.25,color='LightGray')
ax.set_xlabel('Forecast temperature (deg C)',fontsize=11)
ax.set_ylabel('Analyzed temperature (deg C)',fontsize=11)

figname = 'scatter_'+cmonth+'_lon='+clon+'_lat='+clat+'_lead='+clead+'.pdf'
plt.savefig(figname,dpi=400)
print ('Plot done', figname)
plt.close()
